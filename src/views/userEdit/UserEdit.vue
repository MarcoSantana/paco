<template>
  <v-container>
          ...
          <validation-observer ref="form" v-slot="{ invalid, changed }">
            <form @submit.prevent="submit">
              ...
              <v-card-actions class="d-flex justify-center">
                <v-sheet>
                  <v-btn :disabled="invalid || !changed" class="mx-3" color="success" outlined tile @click="save">
                    Guardar cambios
                  </v-btn>
                  <v-btn :disabled="!changed" class="mx-3" color="warning" outlined tile @click="resetForm">
    ...
    <v-snackbar v-model="snackbar.show" absolute centered rounded="pill" color="primary" :timeout="2000">
      {{ snackbar.message }}
      <template v-slot:action="{ attrs }">
        <v-btn color="blue" text v-bind="attrs" @click="snackbar.show = false">{{ $t('actions.close') }}</v-btn>
      </template>
    </v-snackbar>
  </v-container>
</template>
